...
const ru: Dict = {
  ...
  cases: [
    ...
    },
    {
      slug: "aquagrace",
      name: "AquaGrace",
      category: "Сайт школы плавания",
      year: "Personal Brand / Landing",
      stack: ["React", "Tailwind CSS", "Motion", "UI/UX", "Адаптив"],
      intro:
        "Лендинг школы водных видов спорта Дарьи Боковой — плавание, синхронное и русалочье плавание.",
      problem:
        "У тренера не было площадки, где новый ученик мог бы сразу понять форматы занятий, стоимость и записаться, — всё держалось на переписке.",
      task:
        "Собрать персональный сайт-визитку, который передаёт мягкую подачу тренера, снимает страх воды у новичка и ведёт к записи на тренировку.",
      done: [
        "визуальная концепция в водной сине-голубой палитре",
        "hero с портретом и рукописными подписями-акцентами",
        "блок «Обо мне» с метриками опыта",
        "секция «Кому подойдёт»",
        "форматы занятий с ценами и выделением популярного тарифа",
        "блок отзывов учеников",
        "секция контактов и записи",
        "анимации появления и бегущая строка",
        "адаптивная вёрстка под мобильные",
      ],
      process: [
        "Разбор аудитории: новички, взрослые со страхом воды, дети.",
        "Структура лендинга и путь до записи на тренировку.",
        "Дизайн в водной палитре с акцентом на портрет тренера.",
        "Разработка, анимации и адаптив под мобильные устройства.",
      ],
      result:
        "Готовый лендинг персонального бренда: понятные форматы, прозрачные цены и один чёткий путь до записи на тренировку.",
    },
  ],
};
